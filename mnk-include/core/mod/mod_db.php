...
###############################################
        static $dbInfos = array(
            0 => array(
                'server'    =>'db468992172.db.1and1.com',
                'user'      =>'dbo468992172',
                'pw'        =>'22560113',
                'base'      =>'db468992172',
                'type'      =>'mysql'
...
#################################################
    ...
    function dbTime() {
        $dateDB = date("Y-m-j H:i:s");
        return $dateDB;
    }
    
    
    
    function returnEmpty($scanKeyList,$toScan){
        if(is_array($scanKeyList)){
            foreach ($scanKeyList as $key => $val) {
                    if(!array_key_exists($val,$toScan))
                          $toScan[$val]  = "";
                }
        }
